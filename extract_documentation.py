...
class CommentedElement:
    ...
    def get_comment(self, prefix=""):
        return "\n".join(map(lambda x: (prefix + x).rstrip(),
                             self.comment_lines))
...
def write_section(section_name, verb, iface_name, f):
    print("\n\\section " +
          ifacename_to_refname(section_name.lower(), iface_name) + " " +
          section_name, file=f)
    print("\nHere is a list of " + section_name.lower() + " " + verb +
          " by the <tt>" + iface_name + "</tt> interface:\n", file=f)
...
def generate_specific_parameter_list(parameters, args, prefix,
                                     required_direction=None, is_signal=False):
    ...
    for arg in args:
        ...
        elif direction_specific_dbus_type_to_ctype and \
                dbus_type in direction_specific_dbus_type_to_ctype:
            c_type = direction_specific_dbus_type_to_ctype[dbus_type]
        elif len(dbus_type) > 1:
            c_type = "GVariant *"
        else:
            error_exit('Unsupported D-Bus type "' + dbus_type +
                       '" used for argument "' + name + '"')
...
def write_documentation(c_namespace, rettype, prefix, suffix, proxy_typename,
                        generate_parameter_list, members, accept_all_names,
                        hfile):
    for m in members:
        ...
        print(r" * \fn " + rettype + " " + fnname + "(" +
              generate_parameter_list(proxy_typename, m.node) + ")",
              file=hfile)
        print(r" *", file=hfile)
...
def generate_files_from_interfaces(interfaces, component_name, c_namespace,
                                   strip_prefix, hfile, mdfile,
                                   exclude_signals, exclude_methods,
                                   exclude_properties):
...
Interfaces of _""" + component_name + "_:", file=mdfile)
    ...
    if exclude_signals:
        signal_tag_name = None
    if exclude_methods:
        method_tag_name = None
    if exclude_properties:
        property_tag_name = None

    for iface in interfaces:
        print(r"- \subpage " +
              ifacename_to_refname("iface", iface.node.getAttribute("name")),
              file=mdfile)
    ...
    for iface in interfaces:
        ...
        if iface.node.tagName != "interface":
            error_exit('Unexpected tag "' + iface.node.tagName +
                       '", expected interface')
        ...
        all_properties = [m for m in members
                          if m.node.tagName == property_tag_name]

        iface_name = iface.node.getAttribute("name")
        fnname_prefix = \
            ifacename_to_cstyle(strip_prefix_from_interface_name(iface_name,
                                                                 strip_prefix))
        if c_namespace:
            fnname_prefix = c_namespace + "_" + fnname_prefix

        print("\n\n\\page " + ifacename_to_refname("iface", iface_name) +
              " Interface " + iface_name, file=mdfile)

        if iface.comment_lines:
            print("\n" + iface.get_comment(), file=mdfile)

        if len(all_signals) == 0 and len(all_methods) == 0 and \
                len(all_properties) == 0:
            print("\nThis interface is not used by this software.",
                  file=mdfile)
            continue

        proxy_typename = generate_proxy_typename(
                c_namespace, strip_prefix_from_interface_name(iface_name,
                                                              strip_prefix))

        if len(all_signals) > 0:
            write_section("Signals", "emitted", iface_name, mdfile)
            write_table("Signal", fnname_prefix + "_emit",
                        None, all_signals, True, mdfile)
            write_documentation(c_namespace, "void", fnname_prefix + "_emit",
                                None, proxy_typename,
                                generate_signal_emit_parameter_list,
                                all_signals, True, hfile)

        if len(all_methods) > 0:
            write_section("Methods", "implemented", iface_name, mdfile)
            write_table("Method", fnname_prefix + "_call", "sync",
                        all_methods, True, mdfile)
            write_documentation(c_namespace, "gboolean",
                                fnname_prefix + "_call", "sync",
                                proxy_typename,
                                generate_method_call_parameter_list,
                                all_methods, True, hfile)

        if len(all_properties) > 0:
            write_section("Properties", "exposed", iface_name, mdfile)
            write_table("Property", fnname_prefix + "_set", None,
                        all_properties, False, mdfile)
            write_documentation(c_namespace, "void", fnname_prefix + "_set",
                                None, proxy_typename,
                                generate_property_set_parameter_list,
                                all_properties, False, hfile)
...
def main():
    ...
    for o, a in opts:
        if o == "-h":
            ...
            doxygen_header_file = a
        elif o == '-n':
            component_name = a
        elif o == "-o":
            doxygen_markdown_file = a
        elif o == '-s':
            strip_from_interface_name = a
        elif o == '-x':
            if a == 'signals':
    ...
    if not xml_input_file or not doxygen_header_file or \
            not doxygen_markdown_file or not component_name:
        usage()
    ...
    with open(doxygen_header_file, "w+") as hfile, \
         open(doxygen_markdown_file, "w+") as mdfile:
